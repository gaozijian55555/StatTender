...
+(NSArray *)getAllIvarList:(id)instance
{
    ...
    Class endCls=NSClassFromString(@"BaseModelClass");
    while (YES) {
...
+(id)getIvarValueWithObject:(id)instance ivarName:(NSString *)ivarName
{
    ...
    switch (type[0]) {
        case '@':
        {
            ...
            if (reVal) {
                if ([reVal isKindOfClass:[NSArray class]] && [type rangeOfString:@"@\"NSArray" options:NSLiteralSearch].location==0) {
...
//        [self setIvarValue:instance ivarName:name value:data];
//    }
//    return instance;
//}
...
//+(void)setAttributes1:(NSDictionary*)attributes withInstance:(id)instance
//{
...
//        [RunTime setIvarValue:instance ivarName:name value:value];
//    }
//}
                ...
                if ([value isKindOfClass:[NSArray class]] && [type rangeOfString:@"@\"NSArray" options:NSLiteralSearch].location==0) {
                    if (type.length>@"@\"NSArray\"".length) {
                        ...
                        for (NSInteger i=0; i<[list count]; i++) {
                            ...
                            //                        [object performSelectorOnMainThread:@selector(setAttributes:) withObject:[list objectAtIndex:i] waitUntilDone:YES];
                            [arrayObject addObject:object];
                        }
                        value=arrayObject;
                    }else{
                    ...
                    //                [object performSelectorOnMainThread:@selector(setAttributes:) withObject:value waitUntilDone:YES];
                    value=object;
                }
